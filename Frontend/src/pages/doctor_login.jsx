...
const Doctor_Login = () => {
  ...
  const [doctorformData, setDoctorFormData] = useDoctorContext();
  const navigate = useNavigate();
  const vantaRef = useRef(null);
  const vantaEffect = useRef(null);

  useEffect(() => {
    if (!vantaEffect.current && window.VANTA && window.VANTA.BIRDS) {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/doctors/login`,
        doctorData
      );

      const data = response.data;

      // console.log("respponse:", data.Doctor);
      if (data?.Doctor) {
        const normalizedData = {
          id: data.Doctor.id,
          firstName: data.Doctor.name.firstName,
          lastName: data.Doctor.name.lastName,
        ...
        setDoctorFormData(normalizedData);
        console.log(normalizedData);
        localStorage.setItem("doctorData", JSON.stringify(normalizedData)); // ✅ correct key
  ...
  return (
    ...
    >
      ...
      {/* Login Box */}
      <div className="relative z-10 w-full max-w-md p-8 bg-gradient-to-br hover:shadow-[0_4px_20px_rgba(0,0,0,0.5)] from-white/30 to-blue-200/30 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/30">
        <div className="text-center mb-6">
          <h2 className="text-3xl font-extrabold text-white drop-shadow-md">
            Login as <span className="text-blue-200">Doctor</span>
          </h2>
